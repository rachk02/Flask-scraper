...
def create_trend_visualization(structured_trends, default_topic):
    ...
    for topic, trends in structured_trends.items():
        # Trier les tendances par le nombre de posts en ordre décroissant
        sorted_trends = sorted(trends, key=lambda t: t.posts, reverse=True)
        visualization[topic] = {
            "trends": [t.trend for t in sorted_trends],
            "posts": [t.posts for t in sorted_trends],
        }
        # Ajouter toutes les tendances à la liste
        all_trends.extend(sorted_trends)

    # Trier toutes les tendances pour la table
    all_trends_sorted = sorted(all_trends, key=lambda t: t.posts, reverse=True)

    # Générer le graphique
    fig = go.Figure()

    # Afficher uniquement les trends du topic sélectionné par défaut
    selected_trends = visualization.get(default_topic, {})

    fig.add_trace(go.Bar(
        x=selected_trends['posts'],
        y=selected_trends['trends'],
        name=default_topic,
        orientation='h',
        marker=dict(color=colors[default_topic], line=dict(width=1.5, color='black'))
    ))
    ...
    table_data = []
    for trend in all_trends_sorted:
